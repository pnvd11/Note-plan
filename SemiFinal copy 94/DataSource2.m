...
+ (DataSource2 *)sharedInstance
{
    static dispatch_once_t  onceToken;
    static DataSource2 * sSharedInstance;
    
    dispatch_once(&onceToken, ^{
        sSharedInstance = [[DataSource2 alloc] init];
    });
...
- (id)init
{
    ...
    if (self != nil)
    {
        NSURL *dataFileURL = [[self applicationDocumentsDirectory] URLByAppendingPathComponent:@"SavedData2"];
