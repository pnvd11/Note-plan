...
//  xXx
//
//  Created by kuku kay on 11/5/813.
//  Copyright (c) 2013 kuku kay. All rights reserved.
//

#import "DataSource8.h"
@interface DataSource8 ()

@property (nonatomic, strong) NSMutableArray *dataArray;

@end
@implementation DataSource8
+ (DataSource8 *)sharedInstance
{
    static dispatch_once_t  onceToken;
    static DataSource8 * sSharedInstance;
    
    dispatch_once(&onceToken, ^{
        sSharedInstance = [[DataSource8 alloc] init];
    });
...
- (id)init
{
    ...
    if (self != nil)
    {
        NSURL *dataFileURL = [[self applicationDocumentsDirectory] URLByAppendingPathComponent:@"SavedData8"];
